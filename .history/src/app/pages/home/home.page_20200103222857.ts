...
import { TutoInstallIosPage } from 'src/app/modals/tuto-install-ios/tuto-install-ios.page';
...
export class HomePage implements OnInit {
  ...
  ionViewDidEnter() {
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      ...
      const segundaVisita = localStorage.getItem('secondView');
      const pedidos = localStorage.getItem('pedidos');
      e.preventDefault();
      this.deferredPrompt = e;
      this.onAndroid = true;
      if (!segundaVisita && pedidos) {
        this.tutoInstallHidden = false;
        localStorage.setItem('secondView', 'yes');
      }
    });
  ...
  showMenuTuto() {
    ...
    const pedidos = localStorage.getItem('pedidos');
    const showed = localStorage.getItem('tutoReaded');
    if (tipo === 'anonimo' && pedidos === '1' && !showed && this.tutoInstallHidden) {
      this.tutoMenuHidden = false;
      localStorage.setItem('tutoReaded', 'ya');
    }
  }
  ...
  async guardarCalificacion() {
    ...
    this.calificacion.nombre = localStorage.getItem('nombre') || 'Anonimo';
