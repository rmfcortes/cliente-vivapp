...
export class CompraService {
  ...
  async guardarTelefono(tel) {
   ...
   isOpen(): Promise<boolean> {
    return new Promise((resolve, reject) => {
     const opSub = this.db.object(`solo-lectura/abierto`).valueChanges().subscribe((abierto: boolean) => {
       opSub.unsubscribe();
       resolve(abierto);
     });
    });
   ...
   getHorario(): Promise<string> {
    return new Promise((resolve, reject) => {
     const horSub = this.db.object(`solo-lectura/abierto`).valueChanges().subscribe((horario: string) => {
       horSub.unsubscribe();
       resolve(horario);
     });
    });
  ...
  getUbicacionChofer(id) {
    return this.db.object(`carga/${this.uid}/ubicacion`).valueChanges();
  }
