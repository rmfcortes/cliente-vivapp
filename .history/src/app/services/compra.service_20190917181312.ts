...
export class CompraService {
  ...
  constructor(
   ...
   getPedido(): Promise<Pedido> {
     return new Promise((resolve, reject) => {
      const pedidoSub = this.db.object(`usuarios/${this.uid}/pedidos`).valueChanges().subscribe((pedido: Pedido) => {
  ...
  getStatusPedido(id) {
    ...
    return this.db.object(`usuarios/${this.uid}/pedidos/${id}/id`).valueChanges();
  }

  deletePedido(id) {
      localStorage.removeItem('pedido');
      localStorage.removeItem('chofer');
      return this.db.object(`usuarios/${this.uid}/pedidos/${id}`).remove();
  }
